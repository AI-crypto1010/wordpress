...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'pt-ocdi/after_all_import_execution', array( $this, 'regenerate_areas_cache_action' ), 999, 3 );
	...
	public function redux_import( $selected_import_files, $import_files, $selected_index ) {
		if ( ! empty( $selected_import_files['redux'] ) ) {
			PixfortOptionsImporter::import( $selected_import_files['redux'] );
		}
	}
	...
	/**
	 * Regenerate the pixfort dynamic areas cache after full demo import.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `pt-ocdi/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	 */
	public function regenerate_areas_cache_action( $selected_import_files, $import_files, $selected_index ) {
		// Ensure imported popups/templates/headers/etc are available immediately after demo import.
		if ( class_exists( '\\PixfortCore' ) ) {
			$pixfort_core = \PixfortCore::instance();
			if ( isset( $pixfort_core->areasCache ) && is_object( $pixfort_core->areasCache ) && method_exists( $pixfort_core->areasCache, 'regenerate' ) ) {
				$pixfort_core->areasCache->regenerate();
			}
		}
	}

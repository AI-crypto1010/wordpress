...
if (!class_exists('ACF_Module')) {
	class ACF_Module {
		...
		/**
		 * @var ACF_Dynamic_Value_Provider
		 */
			...
			foreach ( $acf_groups as $acf_group ) {
				...
				foreach ( $fields as $field ) {
					...
					if ( $has_option_page_location ) {
						...
						$options[ $key ] = esc_html__( 'Options', 'pixfort-code' ) . ':' . $field['label'];
		...
		public static function add_key_control( $tag ) {
			$tag->add_control(
				'key',
				[
					'label' => esc_html__( 'Key', 'pixfort-code' ),
		...
		// For use by ACF tags
		public static function get_tag_value_field( $tag ) {
			$key = $tag->get_settings( 'key' );

			if ( ! static::$dynamic_value_provider ) {
				static::$dynamic_value_provider = new ACF_Dynamic_Value_Provider();
			}

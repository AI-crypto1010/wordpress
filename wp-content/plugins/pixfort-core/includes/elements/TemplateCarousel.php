...
class PixTemplateCarousel {
	...
	function render($attr, $content = null) {
		...
		if (!empty($slides_arr)) {
			...
			$oldOutput  .= '<div id="' . $pix_id . '" class="pix-main-slider pix-template-carousel ' . $visible_overflow . ' ' . $slider_style . ' ' . $slider_effect . ' ' . $slider_scale . ' ' . $visible_y . ' pix-slider-' . $slider_num . ' pix-slider-dots ' . $dots_style . ' ' . $dots_align . '" ' . $slider_data . '>';
			...
			foreach ($slides_arr as $key => $value) {
				...
				if (!empty($pix_template_id)) {
					...
					$itemOutput .= '<div class="pix-template-carousel-item">';
					
					// Render the Elementor template
					if (class_exists('\Elementor\Plugin')) {
						$pix_template_id = apply_filters('wpml_object_id', $pix_template_id, 'page', true);
						$itemOutput .= \Elementor\plugin::instance()->frontend->get_builder_content_for_display($pix_template_id);
					}
